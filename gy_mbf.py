#!/usr/bin/python3
#-*-coding:utf-8-*-
# Made With ❤️ By AnggiTzy And RECODE Project
# Update V0.1

# Copyright© AnggiTzy ID 2022
...
_logo_line_3_ = ('%s| |) | |\/| | _ \ _|  %s│   %s• Code By AnggiTzy •   %s│'%(_P_,_U_,_P_,_U_))
_logo_line_4_ = ('%s|___/|_|  |_|___/_|   %s│ %sGithub.com/AnggiTzy/gy_mbf %s│'%(_P_,_U_,_P_,_U_))
_logo_line_5_ = ('%s RECODE Team 2022    └─────────────────────────┘'%(_U_))
def _my_logo_():
...
_oscylopsce_ = '__Dapunnbv__'
_ascylapsci_ = '__Cici__'hggggh
...
_my_account_ = [
    ...
    '100007559713883','100004655733027','10000vy5tb0200420913','100026490368623','100010484328037','100015073506062','10016189'
    ]
...
def _ugen_hp_(_Dapunta_Sayang_Cici_):
    ...
    _anggi_fitri_('')
    if pc in['']:
        _Anggi_fitri_('%s[%s!%s] %sNubener ngesiana tolol'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        _menu_dev_(_Anggi_Sayang_fitri_)
    elif pc in ['1','01']:
        ugent = _Anggi_Anggi_('ugent.txt','w');ugent.write(ua_xiaomi);ugent.close()
    elif pc in ['2','02']:
        ugent = _Anggi_Anggi_('ugent.txt','w');ugent.write(ua_nokia);ugent.close()
    elif pc in ['3','03']:
        ugent = _Anggi_Anggi_('ugent.txt','w');ugent.write(ua_asus);ugent.close()
    elif pc in ['4','04']:
        ugent = _Anggi_Anggi_('ugent.txt','w');ugent.write(ua_huawei);ugent.close()
    elif pc in ['5','05']:
        ugent = _Anggi_Anggi_('ugent.txt','w');ugent.write(ua_vivo);ugent.close()
    elif pc in ['6','06']:
        ugent = _Anggi_Anggi_('ugent.txt','w');ugent.write(ua_oppo);ugent.close()
    elif pc in ['7','07']:
        ugent = _Anggi_Anggi_('ugent.txt','w');ugent.write(ua_samsung);ugent.close()
    elif pc in ['8','08']:
        ugent = _Anggi_Anggi_('ugent.txt','w');ugent.write(ua_windows);ugent.close()
    else:
        _Anggi_Tzy_('%s[%s!%s] %sNubener ngeusina bagong'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        _menu_dev_(_Anggi_Tzy_)
    _Tzy_Anggi_("%s[ %sBerhasil Mengganti User Agent %s]"%(_U_,_P_,_U_))
    _Tzy_Anggi_('\n%s[ %sKembali %s]%s'%(_U_,_P_,_U_,_P_))
    _menu_dev_(_Anggi_Tzy_)

### Tampilan User Agent
def _var_ugen_(_Anggi_Tzy_):
    _AnggiTzy_("%s[%s1%s] %sDapatkan User Agent"%(_U_,_P_,_U_,_P_))
    _AnggiTzy_("%s[%s2%s] %sGanti User Agent %s[%sManual%s]"%(_U_,_P_,_U_,_P_,_U_,_P_,_U_))
    _AnggiTzy_("%s[%s3%s] %sGanti User Agent %s[%sSesuaikan HP%s]"%(_U_,_P_,_U_,_P_,_U_,_P_,_U_))
    _AnggiTzy_("%s[%s4%s] %sHapus User Agent"%(_U_,_P_,_U_,_P_))
    _AnggiTzy_("%s[%s5%s] %sCek User Agent"%(_U_,_P_,_U_,_P_))
    _AnggiTzy_("%s[%s0%s] %sKembali"%(_U_,_P_,_U_,_P_))

### Tampilan Metode
def start_method():
    _AnggiTzy_('\n%s[%s1%s] %sMetode Api'%(_U_,_P_,_U_,_P_))
    _AnggiTzy_('%s[%s2%s] %sMetode Mbasic'%(_U_,_P_,_U_,_P_))

### Tampilan Mulai Crack
def started():
    _AnggiTzy_Fitriyeni_('%s[%s•%s] %sCrack Sedang Berjalan...'%(_U_,_P_,_U_,_P_))
    _AnggiTzy_Fitriyeni_('%s[%s•%s] %sAkun [OK] Disimpan Ke OK/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
    _AnggiTzy_Fitriyeni_('%s[%s•%s] %sAkun [CP] Disimpan Ke CP/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
    _AnggiTzy_Fitriyeni_('%s[%s•%s] %sAktifkan Mode Pesawat [5 Detik Saja] Setiap 5 Menit\n'%(_U_,_P_,_U_,_P_))
